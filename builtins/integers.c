...
#define DEFINE_INT_TYPE(c_type, KindOfInt, fmt, min_val, max_val)\
    CORD KindOfInt ## _cord(const c_type *i, bool colorize, const Type *type) { \
        (void)type; \
        CORD c; \
    ...
    String_t KindOfInt ## _format(c_type i, int64_t digits) { \
    ...
    String_t KindOfInt ## _hex(c_type i, int64_t digits, bool uppercase, bool prefix) { \
    ...
    String_t KindOfInt ## _octal(c_type i, int64_t digits, bool uppercase, bool prefix) { \
    ...
    c_type KindOfInt ## _random(c_type min, c_type max) { \
        uint32_t r = arc4random_uniform((uint32_t)(max - min)); \
        return min + (c_type)r; \
    } \
    c_type KindOfInt##_min = min_val, KindOfInt##_max = max_val; \
    Type KindOfInt##_type = { \
        .name=#KindOfInt, \
        .size=sizeof(c_type), \
        .align=alignof(c_type), \
        .tag=VTableInfo, \
        .VTableInfo={.cord=(void*)KindOfInt ## _cord}, \
    };
