...
uint32_t generic_hash(const void *obj, Hashable *what)
{
    switch (what->tag) {
        case HashFunction:
            return what->__data.HashFunction.fn(obj);
        case HashData: {
            uint32_t hash;
            halfsiphash((void*)obj, what->__data.HashData.size, SSS_HASH_VECTOR, (uint8_t*)&hash, sizeof(hash));
            return hash;
        }
        case HashArray: {
            ...
            auto item = what->__data.HashArray.item;

            if (item->tag == HashData) {
                size_t item_size = item->__data.HashData.size;
                ...
                for (unsigned long i = 0; i < s->length; i++) {
                    ...
                    memcpy((p += item_size), &s->data[i*s->stride], item->__data.HashData.size);
                }
                uint32_t hash;
                ...
                for (unsigned long i = 0; i < s->length; i++) {
                    ...
                    *(p++) = generic_hash(&s->data[i*s->stride], item);
                }
                uint32_t hash;
                halfsiphash(&hash_batch, ((int64_t)p) - ((int64_t)hash_batch), SSS_HASH_VECTOR, (uint8_t*)&hash, sizeof(hash));
                return hash;
        ...
        case HashTable: {
            ...
            auto table = what->__data.HashTable;
            ...
            if (t->fallback)
                fallback_hash = generic_hash(t->fallback, what);
