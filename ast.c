...
#define UNOP(u) T(u, F(value))
    ...
    switch (ast->tag) {
        ...
        T(FunctionDef, F(name), F(arg_names), F(arg_types), F(arg_defaults), F(ret_type), F(body), F(is_inline))
        ...
        T(When, F(subject), F(cases), F(default_body))
        T(Skip, F(target))
        T(Stop, F(target))
        UNOP(Return)
        T(Fail, F(message))
        T(Extern, F(name), F(type))
...
static void walk_ast_list(List(ast_t*) ast_list, ast_visitor_t visit, void *userdata, bool depth_first)
{
    LIST_FOR(ast_list, child, _)
        walk_ast(*child, visit, userdata, depth_first);
}
void walk_ast(ast_t *ast, ast_visitor_t visit, void *userdata, bool depth_first)
{
    if (!ast) return;
#define T(tag, ...) case tag: { auto data = Match(ast, tag); (void)data; __VA_ARGS__; break; }
#define F(field) _Generic(data->field, ast_t*: walk_ast((ast_t*)data->field, visit, userdata, depth_first), \
                          List(ast_t*): walk_ast_list((ast_t***)data->field, visit, userdata, depth_first), default: (void)0)
#define BINOP(b) T(b, F(lhs), F(rhs))
#define UNOP(u) T(u, F(value))
    if (!depth_first)
        visit(ast, userdata);
    switch (ast->tag) {
        ...
        T(StructDef, F(name), F(field_names), F(field_types), F(field_defaults), F(definitions))
        T(StructField, F(name), F(value))
        T(TaggedUnionDef, F(name), F(tag_names), F(tag_types))
        T(TypeTaggedUnion, F(name), F(tag_names), F(tag_types))
        T(TaggedUnionField, F(name), F(value))
        T(Index, F(indexed), F(index))
        T(FieldAccess, F(fielded), F(field))
        T(UnitDef, F(derived), F(base))
